<# 
patch_AdminRounds_fix_mojibake.ps1
LPI35++ — Patch “AdminRounds.tsx sem mojibake” + fluxo Git/Vercel.

O que faz:
1) Faz backup do arquivo atual (timestamp) em .\backups\
2) Substitui o AdminRounds.tsx pelo conteúdo “limpo” (UTF‑8 com BOM)
3) (Opcional) roda git status/add/commit/push se você habilitar as flags

Como usar (PowerShell 5.1):
  Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass
  .\patch_AdminRounds_fix_mojibake.ps1 -RepoRoot "C:\Projetos\LIGA35-DEV\liga35-admin" -DoGitCommit -DoGitPush

Observações:
- Ajuste o caminho de destino se seu AdminRounds.tsx estiver em outro local.
- UTF‑8 com BOM reduz chance de mojibake em Windows/VSCode/PowerShell 5.1.
#>

[CmdletBinding()]
param(
  [Parameter(Mandatory = $false)]
  [string]$RepoRoot = "C:\Projetos\LIGA35-DEV\liga35-admin",

  # Se seu arquivo estiver em outro caminho, sobrescreva:
  [Parameter(Mandatory = $false)]
  [string]$TargetRelPath = "src\pages\admin\AdminRounds.tsx",

  # Opcional: comandos Git
  [switch]$DoGitCommit,
  [switch]$DoGitPush,

  # Mensagem de commit
  [string]$CommitMessage = "fix(admin): replace AdminRounds without mojibake"
)
...
$backupPath = Join-Path $backupDir ("AdminRounds.tsx.before_$stamp.bak")

if (Test-Path $targetPath) {
  Copy-Item -Path $targetPath -Destination $backupPath -Force
  Write-Host "Backup criado: $backupPath" -ForegroundColor Green
} else {
  Write-Host "Aviso: alvo não existia ainda, criando novo: $targetPath" -ForegroundColor Yellow
}

# --- Content (AdminRounds.tsx “limpo”)
$NEW_FILE = @'
...
# sanity: garante newline final (boa prática)
if (-not $NEW_FILE.EndsWith("`n")) { $NEW_FILE += "`r`n" }

Write-Utf8BomFile -Path $targetPath -Text $NEW_FILE
Write-Host "Arquivo aplicado (UTF-8 BOM): $targetPath" -ForegroundColor Green
...
try {
  ...
  if (Test-Path (Join-Path $RepoRoot ".git")) {
    ...
    Exec "git add `"$TargetRelPath`""

    if ($DoGitCommit) {
      # se não houver mudanças, commit não faz sentido
      $status = (git status --porcelain)
      if (-not $status) {
        Write-Host "Nada para commitar. Encerrando." -ForegroundColor Yellow
      } else {
        Exec "git commit -m `"$CommitMessage`""
        Write-Host "Commit OK." -ForegroundColor Green
      }
    } else {
      Write-Host "Git commit não executado (use -DoGitCommit para habilitar)." -ForegroundColor Yellow
    }

    if ($DoGitPush) {
      Exec "git push"
      Write-Host "Push OK." -ForegroundColor Green
      Write-Host "Vercel: se seu projeto está conectado ao Git, o deploy dispara automaticamente." -ForegroundColor Green
    } else {
      Write-Host "Git push não executado (use -DoGitPush para habilitar)." -ForegroundColor Yellow
      Write-Host "Vercel: faça push no branch configurado para disparar o deploy." -ForegroundColor Yellow
    }
  } else {
    Write-Host "RepoRoot não parece ter .git. Pulei Git. (Se for monorepo, rode no root correto.)" -ForegroundColor Yellow
  }
}
finally {
  Pop-Location
...
Write-Host "1) Abra o projeto no VS Code e confira se acentos estão OK." -ForegroundColor White
Write-Host "2) Rode: npm install (se necessário) e npm run dev para validar." -ForegroundColor White
Write-Host "3) Se Vercel conectado: push no branch -> deploy automático." -ForegroundColor White
