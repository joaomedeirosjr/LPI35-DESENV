  ...
  async function gerarLote() {
    ...
    if (list.length > 0) {
      const links = list
        ...
        .join('\n')
      try { await navigator.clipboard.writeText(links) } catch {}
  ...
  return (
    <div className='space-y-6 text-white'>
      <div className='flex items-start justify-between gap-4'>
        ...
        <button className='btn-ghost' onClick={load} disabled={loading || busy}>
      ...
      <div className='grid gap-4 md:grid-cols-3'>
        <div className='card space-y-3 md:col-span-2'>
          ...
          <div className='flex flex-wrap gap-2'>
            <button className='btn-primary' onClick={gerarUm} disabled={busy}>
              {busy ? 'Gerando...' : 'Gerar 1 (copia link)'}
            </button>
            <button className='btn-ghost' onClick={gerarLote} disabled={busy}>
        ...
        <div className='card space-y-2'>
          ...
          <div className='text-xs text-slate-400'>
            Mostrando: {rows.length} convites
          </div>
        </div>
      </div>
              ...
              <table className='w-full text-sm'>
                <thead className='text-slate-300'>
                  <tr className='border-b border-white/10'>
                    ...
                    <th className='text-left py-3 px-3'>Validade</th>
                    <th className='text-left py-3 px-3'>Uso</th>
                    <th className='text-left py-3 px-3'>Utilizado por</th>
                  ...
                  {rows.map((r) => (
                    <tr key={r.id} className='border-b border-white/5 align-top'>
                      <td className='py-3 px-3'>
                        <span className={'inline-flex items-center px-2 py-1 rounded-lg border text-xs ' + statusStyle(r.status)}>
                          {r.status}
                        </span>
                      </td>

                      <td className='py-3 px-3'>
                        <div className='font-semibold'>{shortToken(r.token)}</div>
                        <div className='text-xs text-slate-400'>ID: {r.id}</div>
                      </td>

                      <td className='py-3 px-3'>
                        <div className='text-slate-200'>Expira: {fmt(r.expires_at)}</div>
                        <div className='text-xs text-slate-400'>Criado: {fmt(r.created_at)}</div>
                      </td>

                      <td className='py-3 px-3'>
                        <div className='text-slate-200'>{r.uses}/{r.max_uses}</div>
                        {r.used_at ? <div className='text-xs text-slate-400'>em {fmt(r.used_at)}</div> : <div className='text-xs text-slate-400'></div>}
                      </td>
                      ...
                      <td className='py-3 px-3'>
                        <div className='flex justify-end gap-2'>
                          <button className='btn-ghost' onClick={() => copiarLink(r.token)} disabled={busy}>
                            Copiar link
                          </button>
